...
public partial class MainWindow : Window
{
    ...
    {
        var loadingPanel = new Grid
        {
            VerticalAlignment = Avalonia.Layout.VerticalAlignment.Center,
            HorizontalAlignment = Avalonia.Layout.HorizontalAlignment.Center
        };

        // Фоновое изображение (опционально - можно использовать другую картинку)
        var backgroundImage = new Border
        {
            Width = 600,
            Height = 400,
            ...
            Background = new LinearGradientBrush
            {
                ...
                GradientStops =
                {
                    new GradientStop(Color.FromArgb(200, 20, 40, 80), 0),
                    new GradientStop(Color.FromArgb(200, 40, 60, 100), 1)
                }
            },
        ...
        var titleText = new TextBlock
        {
            ...
            Foreground = new LinearGradientBrush
            {
                ...
                GradientStops =
                {
                    new GradientStop(Color.FromRgb(100, 200, 255), 0),
                    new GradientStop(Color.FromRgb(255, 255, 255), 0.5),
                    new GradientStop(Color.FromRgb(100, 200, 255), 1)
                }
            },
            HorizontalAlignment = Avalonia.Layout.HorizontalAlignment.Center,
        ...
        {
            Text = "Game by F.A.S.T DEVELOPMENT",
            FontSize = 16,
            Foreground = new SolidColorBrush(Color.FromRgb(150, 180, 220)),
            HorizontalAlignment = Avalonia.Layout.HorizontalAlignment.Center,
        ...
        // Прогресс-бар
        var progressBarBackground = new Border
        {
            Width = 400,
            Height = 20,
        ...
        var progressBarFill = new Border
        {
            ...
            Background = new LinearGradientBrush
            {
                ...
                GradientStops =
                {
                    new GradientStop(Color.FromRgb(50, 150, 255), 0),
                    new GradientStop(Color.FromRgb(100, 200, 255), 1)
                }
            },
        ...
        progressGrid.Children.Add(progressBarBackground);
        ...
        contentStack.Children.Add(progressGrid);

        backgroundImage.Child = contentStack;
        loadingPanel.Children.Add(backgroundImage);

        Content = loadingPanel;

        // СИМУЛЯЦИЯ ЗАГРУЗКИ (можно заменить на реальную инициализацию)
        await SimulateLoadingAsync(progressBarFill, statusText);

        // После загрузки показываем главное меню
        ShowMainMenu();
    }
    
    private async Task SimulateLoadingAsync(Border progressBar, TextBlock statusText)
    {
        ...
        foreach (var (status, progress) in loadingSteps)
        {
            ...
            var targetWidth = 400.0 * progress / 100;
            ...
            {
                progressBar.Width = currentWidth + increment * (i + 1);
                await Task.Delay(30); // 30ms * 20 шагов = ~600ms на этап
            }

            await Task.Delay(200); // Небольшая пауза между этапами
        }

        statusText.Text = "Готово! ✓";
        await Task.Delay(300); // Показываем "Готово" перед переходом в меню
    }
    ...
    private void HandleChatMessage(Dictionary<string, string> data)
    {
        ...
        chatMessages.Add((sender, text, DateTime.Now));
    ...
    private async Task SendChatMessageAsync(string text)
    {
        ...
        chatMessages.Add(("Вы", text, DateTime.Now));
        UpdateChatDisplay();
    }
    ...
    private void UpdateChatDisplay()
    {
        ...
        foreach (var (sender, text, timestamp) in chatMessages)
        {
            ...
            // Верхняя строка: отправитель и время
            ...
            var senderBlock = new TextBlock
            {
                ...
                Foreground = sender == "Вы" 
                    ? new SolidColorBrush(Color.FromRgb(144, 238, 144)) // LightGreen
                    : new SolidColorBrush(Color.FromRgb(135, 206, 250)), // LightSkyBlue
                FontSize = 13
            };
            ...
            chatPanel.Children.Add(separator);
        }
    ...
    private StackPanel CreateChatPanel()
    {
        ...
        chatScrollViewer = new ScrollViewer
        {
            ...
            BorderBrush = new SolidColorBrush(Color.FromArgb(100, 100, 150, 200)),
            BorderThickness = new Thickness(1)
        };
        ...
        chatInputBox = new TextBox
        {
            ...
            Background = new SolidColorBrush(Color.FromRgb(35, 45, 65)),
            Foreground = Brushes.Black,
            BorderBrush = new SolidColorBrush(Color.FromRgb(70, 90, 130)),
            BorderThickness = new Thickness(1),
